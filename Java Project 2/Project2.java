...
public class Project2 {
    ...
    public int index(Complex root) {
        ...
        for (int p=0; p <= roots.size() - 1; p++) {
            if ((roots.get(p).subtract(root)).abs() < Secant.TOL) {
                return p;
            }
        }
        return -1;
    ...
    double z = width / NUMPIXELS;
    Complex jik = new Complex (i,-j);
    Complex zjik = jik.multiply(z);
    Complex Num = origin.add(zjik);
    return Num;
    }
    ...
    public void createFractal(boolean colourIterations) {
        ...
        for (int j=0; j <= NUMPIXELS - 1; j++) {
            for (int k=0; k <= NUMPIXELS - 1; k++) {
                Complex MyNumber = pixelToComplex(j, k);
                Complex zero = new Complex();
                iterator.iterate(zero, MyNumber);

                if (iterator.getError() == Secant.Error.OK)  {
                    if (index(iterator.getRoot()) == -1) {
                        roots.add(iterator.getRoot());
                        colourPixel(j,k,index(iterator.getRoot()), iterator.getNumIterations());
                    }
    ...
    public static void main(String[] args) {
        ...
        Complex[] coeff = new Complex[] { new Complex(1.0,1.0),  new Complex(2.5,1), new Complex(3.5,2), new Complex(1.0,2.0) };
